...
class FileService:
    ...
    def _effective_max_file_size(self) -> tuple[int, str]:
        """Лимит размера файла из runtime-конфига (операторский параметр)."""
        raw = self._runtime.get("max_file_size")
        try:
    ...
    async def save_upload(self, upload: UploadFile) -> StoredFile:
        ...
        total_size = 0
        max_bytes, max_label = self._effective_max_file_size()
                ...
                while chunk := await upload.read(1024 * 1024):
                    ...
                    if total_size > max_bytes:
                        raise FileTooLargeError(
                            f"Размер файла превышает лимит {max_label}."
                        )
                    buffer.write(chunk)
