...
капотом, ручного обновления оператором не требуется (подход заимствован из
`cases/ai-curator/src/services/gigachat_adapter.py`).

Возвращает сырой текст ответа + usage — без structured_output (GigaChat не
поддерживает json_schema strict), поэтому `AIService` прогоняет результат через
устойчивый парсер `_parse_plan` (free-text fallback).
