...
from app.services.registries import CHART_TYPES_SET
...
class ChartService:
    ...
    def generate_chart(
        ...
        if chart_type not in CHART_TYPES_SET:
            raise FileReadError(f"Поддерживаются только графики: {', '.join(sorted(CHART_TYPES_SET))}.")
    ...
    def _generate_table_chart(
        ...
        if chart_type == "histogram":
            ...
            if numeric_columns:
                pie_x = x_column or (dimension_columns or list(dataframe.columns))[0]
                pie_y = y_column or numeric_columns[0]
                grouped = (
                    dataframe[[pie_x, pie_y]]
                    .copy()
                    .dropna(subset=[pie_x, pie_y])
                    .groupby(pie_x, dropna=True)[pie_y]
                    .sum()
                ...
                description = f"Доли «{pie_y}» по категориям «{pie_x}» (сумма)."
            else:
                pie_x = x_column or (dimension_columns or list(dataframe.columns))[0]
                pie_y = None
                grouped = dataframe[pie_x].astype(str).value_counts().head(10)
                if grouped.empty:
                    raise FileReadError("Недостаточно данных для pie графика.")
                description = f"Доли по категориям «{pie_x}» (частоты)."
            axis.pie(
                grouped.values,
            ...
            axis.set_title(f"Pie chart: {pie_x}")
            axis.axis("equal")
        else:
            if numeric_columns:
    ...
    def _generate_image_chart(self, stored_file: StoredFile, chart_type: str) -> dict[str, Any]:
        ...
        if chart_type == "histogram":
            ...
            if array.ndim == 2:
                ...
                labels = list(image.getbands())
                values = [array[:, :, index].mean() for index in range(array.shape[2])]
            pie_palette = ["#d06b4e", "#114b5f", "#6c8b6b", "#8d3f28", "#c9a86a"][: len(labels)]
            axis.pie(values, labels=labels, autopct="%1.1f%%", startangle=90, colors=pie_palette)
