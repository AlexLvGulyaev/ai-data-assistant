...
class ChatService:
        ...
        for action in actions:
            ...
            try:
                if action_type == "preview":
                    ...
                    if analysis_cache is None:
                        analysis_cache = self.analysis_service.analyze(active_file)
                    existing_charts = self.file_service.get_output_artifacts(active_file.file_id)["charts"]
                    chart_records = self._diverse_chart_records(active_file, existing_charts)
    ...
    def _handle_local_prompt(
        ...
        if action == "report":
            try:
                ...
                existing_charts = self.file_service.get_output_artifacts(active_file.file_id)["charts"]
                chart_records = self._diverse_chart_records(active_file, existing_charts)
                report = self.report_service.generate_report(active_file, analysis, chart_records, preview)
            except FileServiceError as exc:
    ...
    def _chart_type_from_name(self, file_name: str) -> str | None:
        """Имя артефакта: `{file_id}__{chart_type}__{timestamp}.png` → chart_type."""
        parts = file_name.split("__")
        if len(parts) >= 3:
            return parts[1].lower()
        return None

    def _diverse_chart_records(
        self,
        stored_file: StoredFile,
        existing_records: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Графики для DOCX-отчёта: по одному каждого типа (новейший на диске),
        недостающие типы из `REPORT_CHART_ORDER` добираются генерацией.

        Решает проблему «в отчёт попали три одинаковых pie»: берём максимум
        по одному на тип, prioritizing разнообразие, а не свежесть на диске.
        """
        # existing_records отсортированы file_service'ом по свежести (newest-first).
        newest_by_type: dict[str, dict[str, Any]] = {}
        for record in existing_records:
            chart_type = self._chart_type_from_name(record.get("file_name", ""))
            if chart_type and chart_type not in newest_by_type:
        ...
        for chart_type in ordered[:3]:
            if chart_type in newest_by_type:
                ...
                continue
            try:
                generated = self.chart_service.generate_chart(stored_file, chart_type)
            except FileServiceError:
                # Тип неприменим к данным файла (например, нет числовых колонок) — пропускаем.
                continue
